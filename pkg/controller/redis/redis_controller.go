...
import (
	...
	"math/rand"
	"redis/pkg/resources/configmap"
	"redis/pkg/resources/job"
	"redis/pkg/resources/service"
	"redis/pkg/resources/statefulset"
	"reflect"
	"strconv"
	"strings"
	"time"
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &crdv1alpha1.Redis{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
func (r *ReconcileRedis) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil && errors.IsNotFound(err) {
		...
		*/
		...
		instance.Annotations = map[string]string{
			"crd.custom.local/spec":toString(instance),
		}
	...
	//instance.Annotations["crd.custom.local/spec"]这是老的信息
	//instance.spec是期望的最新的信息，使用DeepEqual方法比较是否相等
	if ! reflect.DeepEqual(instance.Spec,toSpec(instance.Annotations["crd.custom.local/spec"])) {
		//如果不相等，就需要去更新，更新就是重建sts和svc
		//但是更新操作通常是不会去更新svc的，只需要更新sts
		oldClusterSize := fmt.Sprintf("%v",*(toSpec(instance.Annotations["crd.custom.local/spec"]).Replicas))
...
func toString(redisCluster *crdv1alpha1.Redis) string {
	bytes, _ := json.Marshal(redisCluster.Spec)
	return  string(bytes)
}

func toSpec(data string) crdv1alpha1.RedisSpec {
	redisClusterSpec := crdv1alpha1.RedisSpec{}
